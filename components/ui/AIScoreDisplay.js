    ...
    return (
        <div className="glass-card p-5 space-y-4">
            <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-xs font-semibold text-white/60 uppercase tracking-wider">AI Match Analysis</span>
            </div>
            ...
            {Object.keys(breakdown).length > 0 && (
                <div className="space-y-1.5">
                    <span className="text-xs text-white/40 uppercase tracking-wide">Score Breakdown</span>
                    {Object.entries(breakdown).map(([key, val]) => (
                        <div key={key} className="flex items-center gap-3">
                            <span className="text-xs text-white/50 w-28 capitalize">{key.replace('Score', '')}</span>
